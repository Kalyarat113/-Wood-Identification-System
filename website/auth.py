    ...
    if request.method == 'POST':
        ...
        if user:
            if check_password_hash(user.password, password):
                ...
                flash('Incorrect password, try again.', category='error')
                # คืนฟอร์มแบบมีข้อมูลผู้ใช้กรอกมาแล้ว
                return render_template("login.html", user=current_user, site_key=create_app().config['RECAPTCHA_SITE_KEY'], email=email)
        else:
            flash('Email does not exist.', category='error')
            # คืนฟอร์มแบบมีข้อมูลผู้ใช้กรอกมาแล้ว
            return render_template("login.html", user=current_user, site_key=create_app().config['RECAPTCHA_SITE_KEY'], email=email)
...
def sign_up():
    ...
    if request.method == 'POST':
        ...
        return render_template("sign_up.html", user=current_user, roles=sorted_roles, site_key=create_app().config['RECAPTCHA_SITE_KEY'], email=email, first_name=first_name, last_name=last_name, user_name=user_name, role_id=role_id)
...
def update_profile():
    ...
    if errors:
        for error in errors:
            flash(error, category='error')
        return render_template('profile.html', user=user, roles=Role.query.all(), edit=True, email=email, first_name=first_name, last_name=last_name, user_name=user_name, role_id=role_id)
